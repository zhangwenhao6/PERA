...
class LoraLayer:
    def __init__(
        ...
        self.A_parameter_square = nn.ParameterDict({})
        self.lora_A_parameter_cross = nn.ParameterDict({})
        self.lora_B = nn.ParameterDict({})
        # self.lora_B_parameter = nn.ParameterDict({})
        # For Embedding layer
    ...
    def update_layer(self, adapter_name, r_ab, lora_alpha, lora_dropout, init_lora_weights, scale_ab, init_a,
        ...
        if r_ab > 0:
            ...
            self.A_parameter_square.update(nn.ParameterDict({adapter_name: nn.Parameter(torch.randn(int(r_ab)))}))
            self.lora_A_parameter_cross.update(nn.ParameterDict({adapter_name: nn.Parameter(torch.randn(int(r_ab*(r_ab-1)//2)))}))
            self.lora_B.update(nn.ParameterDict({adapter_name: nn.Parameter(torch.randn(self.out_features, r_ab))}))
            # self.lora_B_parameter.update(nn.ParameterDict({adapter_name: nn.Parameter(torch.randn(1,int((r_ab + 2) * (r_ab + 1) // 2 -1 -r_ab)))}))
    ...
    def reset_lora_parameters(self, adapter_name):
        ...
        if adapter_name in self.lora_A.keys() or adapter_name in self.lora_C.keys():
            ...
            if self.r_ab[adapter_name] > 0:
                ...
                nn.init.zeros_(self.A_parameter_square[adapter_name])
                nn.init.zeros_(self.lora_A_parameter_cross[adapter_name])
                # nn.init.ones_(self.lora_B_parameter[adapter_name])
                # AB prod
...
class Linear(nn.Linear, LoraLayer):
    ...
    def torch_polynomial_features_for_matrix_encoder(self, X, include_bias=False):
        ...
        squared_cross__feats = prod[:, indices[0], indices[1]]    # (n_samples, n_cross_terms)

        # 区分平方项和交叉项
        squared_mask = (indices[0] == indices[1])
        cross_mask = (indices[0] < indices[1])

        squared_cross__feats[:, squared_mask] *= self.A_parameter_square[self.active_adapter]
        squared_cross__feats[:, cross_mask] *= self.lora_A_parameter_cross[self.active_adapter]

        features.append(squared_cross__feats)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
            lora_A_Polynomial = self.torch_polynomial_features_for_matrix_encoder(self.lora_A[self.active_adapter].T,include_bias=False).T
            lora_B_Polynomial = self.torch_polynomial_features_for_matrix_decoder(self.lora_B[self.active_adapter],include_bias=False)

            output_1 = F.linear(self.lora_dropout[self.active_adapter](x),transpose(lora_A_Polynomial,self.fan_in_fan_out),bias=self.bias)
            output_2 = F.linear(output_1,transpose(lora_B_Polynomial,self.fan_in_fan_out),bias=self.bias)
