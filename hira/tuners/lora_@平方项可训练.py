...
class LoraLayer:
    def __init__(
        ...
        self.lora_A_parameter_square = nn.ParameterDict({})
        self.A_parameter_cross = nn.ParameterDict({})
    ...
    def update_layer(self, adapter_name, r_ab, lora_alpha, lora_dropout, init_lora_weights, scale_ab, init_a,
        ...
        if r_ab > 0:
            ...
            self.lora_A_parameter_square.update(nn.ParameterDict({adapter_name: nn.Parameter(torch.randn(int(r_ab)))}))
            self.A_parameter_cross.update(nn.ParameterDict({adapter_name: nn.Parameter(torch.randn(int(r_ab*(r_ab-1)//2)))}))
    ...
    def reset_lora_parameters(self, adapter_name):
        ...
        if adapter_name in self.lora_A.keys() or adapter_name in self.lora_C.keys():
            ...
            if self.r_ab[adapter_name] > 0:
                ...
                nn.init.zeros_(self.lora_A_parameter_square[adapter_name])
                nn.init.zeros_(self.A_parameter_cross[adapter_name])
...
class Linear(nn.Linear, LoraLayer):
    ...
    def torch_polynomial_features_for_matrix_encoder(self, X, include_bias=False):
        ...
        squared_cross__feats[:, squared_mask] *= self.lora_A_parameter_square[self.active_adapter]
        squared_cross__feats[:, cross_mask] *= self.A_parameter_cross[self.active_adapter]
