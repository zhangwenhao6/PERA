...
class LoraLayer:
    def __init__(
        ...
        self.lora_A_parameter = nn.ParameterDict({})
    ...
    def update_layer(self, adapter_name, r_ab, lora_alpha, lora_dropout, init_lora_weights, scale_ab, init_a,
        ...
        if r_ab > 0:
            ...
            self.lora_A_parameter.update(nn.ParameterDict({adapter_name: nn.Parameter(torch.randn(int((r_ab + 2) * (r_ab + 1) // 2 -1 -r_ab)))}))
    ...
    def reset_lora_parameters(self, adapter_name):
        ...
        if adapter_name in self.lora_A.keys() or adapter_name in self.lora_C.keys():
            ...
            if self.r_ab[adapter_name] > 0:
                ...
                nn.init.zeros_(self.lora_A_parameter[adapter_name])
                nn.init.zeros_(self.lora_B_parameter[adapter_name])
                # AB prod
...
class Linear(nn.Linear, LoraLayer):
    ...
    def torch_polynomial_features_for_matrix_decoder(self, X, include_bias=False):
        ...
        # 取严格上三角部分（包括对角线）
        indices = torch.triu_indices(n_features, n_features, offset=0)
        cross_feats = prod[:, indices[0], indices[1]] * self.lora_B_parameter[self.active_adapter]  # (n_samples, n_cross_terms)
        features.append(cross_feats)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            result = F.linear(self.lora_dropout[self.active_adapter](x), transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
            ...
            output_1 = F.linear(x,transpose(lora_A_Polynomial,self.fan_in_fan_out),bias=self.bias)
